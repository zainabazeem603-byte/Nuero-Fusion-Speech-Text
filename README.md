...
The full Neuro Fusion-RAG pipeline also has:
- An **MRI branch** (Swin Transformer, 4-class dementia staging) — not part of this repo.
- A **fusion branch** combining acoustic + speech + text features — not deployed here,
  because the original 47-dim acoustic feature extractor used at training time could
  not be confirmed from the project files. Only the two unimodal branches (text-only,
  speech-only), which don't need the acoustic features, are deployed.
